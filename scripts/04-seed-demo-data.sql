...
-- A fixed id so the accounts below can reference it without a lookup.
INSERT INTO tenants (id, name, domain, status) VALUES
  ('11111111-1111-1111-1111-111111111111', 'Demo Company', 'demo.localhost', 'active')
...
VALUES
  (
    ...
    '22222222-2222-2222-2222-222222222222',
    '22222222-2222-2222-2222-222222222222',
    '{"sub":"22222222-2222-2222-2222-222222222222","email":"admin@demo.localhost","email_verified":true,"phone_verified":false}',
    'email', NOW(), NOW(), NOW()
  ),
  (
    gen_random_uuid(),
    '33333333-3333-3333-3333-333333333333',
    '33333333-3333-3333-3333-333333333333',
    '{"sub":"33333333-3333-3333-3333-333333333333","email":"member@demo.localhost","email_verified":true,"phone_verified":false}',
    'email', NOW(), NOW(), NOW()
  )
...
INSERT INTO tasks (id, tenant_id, project_id, title, description, status, priority, assignee_id, due_date) VALUES
  ('66666666-6666-6666-6666-666666666601', '11111111-1111-1111-1111-111111111111', '44444444-4444-4444-4444-444444444444',
   'Audit the current pages', 'List every page and what replaces it.', 'completed', 'medium',
   '33333333-3333-3333-3333-333333333333', CURRENT_DATE - INTERVAL '1 week'),
  ('66666666-6666-6666-6666-666666666602', '11111111-1111-1111-1111-111111111111', '44444444-4444-4444-4444-444444444444',
   'Build the component library', NULL, 'in_progress', 'high',
   '33333333-3333-3333-3333-333333333333', CURRENT_DATE + INTERVAL '1 week'),
  ('66666666-6666-6666-6666-666666666603', '11111111-1111-1111-1111-111111111111', '44444444-4444-4444-4444-444444444444',
   'Migrate the blog', NULL, 'todo', 'low',
   NULL, CURRENT_DATE + INTERVAL '3 weeks'),
  ('66666666-6666-6666-6666-666666666604', '11111111-1111-1111-1111-111111111111', '55555555-5555-5555-5555-555555555555',
   'Pick the navigation pattern', NULL, 'todo', 'medium',
   '22222222-2222-2222-2222-222222222222', CURRENT_DATE + INTERVAL '2 weeks'),
  -- Deliberately past due, so the dashboard's overdue count is not always zero.
  ('66666666-6666-6666-6666-666666666605', '11111111-1111-1111-1111-111111111111', '44444444-4444-4444-4444-444444444444',
   'Write the launch checklist', NULL, 'in_review', 'urgent',
   '22222222-2222-2222-2222-222222222222', CURRENT_DATE - INTERVAL '2 days')
ON CONFLICT (id) DO NOTHING;
